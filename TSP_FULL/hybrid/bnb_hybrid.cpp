...
#pragma omp parallel for
					...
					{
						T2 sol = data[j];
						//ignore if its bound is worse than already known best solution
						if ((prb.get_goal() && (sol.get_bound() < best_cost)) || (!prb.get_goal() && (sol.get_bound() > best_cost)))
							continue;
